<?php
namespace hehe\core\hroute;

use hehe\core\hroute\base\GroupRule;
use hehe\core\hroute\base\MatchingResult;
use hehe\core\hroute\base\RouteCache;
use hehe\core\hroute\base\RouteMatcher;
use hehe\core\hroute\base\RouteRequest;
use hehe\core\hroute\base\Rule;
use hehe\core\hroute\fast\FastRouteMatcher;
use hehe\core\hroute\threads\ProcessModel;
...
class RouteManager
{
     ...
     * @var ProcessModel
     */
    protected $threadLocal = ProcessModel::class;
    ...
    public function __construct(array $config = [])
    {
        ...
        $this->threadLocal = $this->createThreadModel();
    }

    protected function createThreadModel()
    {
        return new $this->threadLocal();
    }

    protected function arrRulesToObjRules(array $rules):array
    {
        $ruleList =  [];
        foreach ($rules as $key=>$rule) {
            if (is_array($rule)) {
                $ruleList[] = static::createRule($rule);
            } else if (is_string($key) && is_string($rule)) {
                $ruleList[] = static::createRule($key, $rule);
     ...
     * 路由规则注入路由解析器
     */
    protected function injectRules(): void
    {
        $injectResult = false;
     ...
     * @return bool
     */
    protected function injectRouteCache(): bool
    {
     ...
     * 路由缓存写入文件
     */
    protected function writeRouteCache():void
    {
        if ($this->onRouteCache) {
            $routeCache = $this->getRouteCache();
            if (!$routeCache->checkCacheStatus()) {
     ...
     * 注入路由规则至路由解析器
     */
    protected function ruleAddtoRouteMatcher():void
    {
        /** @var Rule[] $rules */
        $rules = [];
        if (!empty($this->rules)) {
            $rules = array_merge($rules,$this->arrRulesToObjRules($this->rules));
            $this->rules = [];
        }
    ...
    public function get(string $uri = '',string $action = '',array $options = []):Rule
    {
        return $this->addRoute($uri,$action,Route::GET_METHOD,$options);
    }

    public function post(string $uri = '',string $action = '',array $options = []):Rule
    {
        return $this->addRoute($uri,$action,Route::POST_METHOD,$options);
    }

    public function put(string $uri = '',string $action = '',array $options = []):Rule
    {
        return $this->addRoute($uri,$action,Route::PUT_METHOD,$options);
    }

    public function patch(string $uri = '',string $action = '',array $options = []):Rule
    {
        return $this->addRoute($uri,$action,Route::PATCH_METHOD,$options);
    }

    public function delete(string $uri = '',string $action = '',array $options = []):Rule
    {
        return $this->addRoute($uri,$action,Route::DELETE_METHOD,$options);
    }

    public function head(string $uri = '',string $action = '',array $options = []):Rule
    {
        return $this->addRoute($uri,$action,Route::HEAD_METHOD,$options);
    }

    public function any(string $uri = '',string $action = '',array $options = []):Rule
    {
    ...
    public function getRouteMatcher():RouteMatcher
    {
        ...
        if (isset($routeMatcherConfig['class']) && strpos($routeMatcherConfig['class'],'\\') !== false) {// 采用命名空间
            ...
            unset($routeMatcherConfig['class']);
        } else {
    ...
    {
        if (!is_null($this->_routeMatcher)) {
            $this->_routeMatcher->setOptions($routeMatcherConfig);
        }
    ...
    public function setRouteCache(array $routeCache): self
    {
        if (!is_null($this->_routeCache)) {
            $this->_routeCache->setOptions($routeCache);
        }
    ...
    public function parseRequest($routeRequest = null):MatchingResult
    {
        ...
        $this->threadLocal->routeRequest = $routeRequest;
     ...
     * @return string
     */
    public function buildUrL($url = '',array $params = [],array $options = []):string
    {
        // 注入路由规则
        $this->injectRules();

        if (is_bool($url) && $url === true && !is_null($this->threadLocal->routeRequest)) {
            $matchingResult = $this->threadLocal->routeRequest->getMatchingResult();
            $url = $matchingResult->getAction();
            $params = array_merge($matchingResult->getParams(),$params);
        } else if ($url === '' && !is_null($this->threadLocal->routeRequest)) {
            $matchingResult = $this->threadLocal->routeRequest->getMatchingResult();
            $url = $matchingResult->getAction();
        }
