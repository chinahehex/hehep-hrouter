<?php
namespace hehe\core\hroute\base;
...
abstract class RouteMatcher
{
    ...
    public function runCallable(GroupRule $rule):void
    {
        $rule->runCallable();
    }
     ...
     * @return array|null
     */
    public function matchMergeUriRules(array $rules = [],?RouteRequest $routeRequest = null,string $cacheKey = '',int $mergeLen = 0):?array
    {
        if (empty($rules)) {
            return null;
        }
     ...
     * @return array|null
     */
    public function matchMergeActionRules(array $rules = [],string $uri = '',array $params = [],string $cacheKey = '',int $mergeLen = 0):?array
    {
        if (empty($rules)) {
            return null;
        }
     ...
     */
    public function matchUriRules(array $rules = [],?RouteRequest $routeRequest = null):?array
    {
        $matchResult = null;
        if (!empty($rules)) {
            foreach ($rules as $rule) {
     ...
     * @return array
     */
    public function matchActionRules(?array $rules,string $uri,array $params = []):?array
    {
        $matchResult = null;
        if (!empty($rules)) {
            foreach ($rules as $rule) {
     ...
     * @return string
     */
    public function buildUrL(string $uri = '',array $params = [],array $options = []):string
    {
     ...
     * @return bool|array
     */
    abstract public function matchRequest(RouteRequest $routeRequest):?array;
     ...
     * @return array|null
     */
    abstract public function matchAction(string $uri = '',array $params = [],array $options = []):?array;
